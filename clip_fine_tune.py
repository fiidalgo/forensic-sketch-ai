...
class SketchCaptionDataset(Dataset()):
    def __init__(self, sketches_dir, captions_dir, processor):
        ...
        # Sync both lists
        valid_sketch_files, valid_caption_files = [], []
        i, j = 0, 0
...
class LoRALinear(nn.Module):
    def __init__(self, original_linear, r, lora_alpha, lora_dropout):
        ...
        # Initialize B to zeros for better training stability
    ...
    def forward(self, x):
        ...
        if self.r > 0:
            lora_res = (self.lora_dropout(x) @ self.lora_A @ self.lora_B) * self.scaling
            return res + lora_res
        return res
...
def evaluate(model, dataloader, k_vals=[1, 5, 10, 25]):
    model.eval()
...
def plot_training_metrics(metrics, save_dir="training_plots"):
    ...
    plt.title('Training Loss over Epochs for CLIP Fine-Tuning')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.grid(True)
    plt.legend()
    ...
    plt.title('Top-K Validation Accuracy over Epochs for CLIP Fine-Tuning')
    plt.xlabel('Epoch')
    plt.ylabel('Accuracy')
    plt.grid(True)
    plt.legend()
...
def main():
    ...
    captions_dir = os.path.join(data_dir, "descriptions")
    ...
    lora_params = apply_lora(model, r=16, lora_alpha=32, lora_dropout=0.1)
    model = model.to(device)

    # Prepare dataset
    dataset = SketchCaptionDataset(sketches_dir, captions_dir, processor)
